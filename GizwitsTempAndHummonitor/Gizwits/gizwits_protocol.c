...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagbuzzer)
    {
        info->event[info->num] = EVENT_buzzer;
        info->num++;
        dataPoints->valuebuzzer = gizStandardDecompressionValue(buzzer_BYTEOFFSET,buzzer_BITOFFSET,buzzer_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagalarm_mode)
    {
        info->event[info->num] = EVENT_alarm_mode;
        info->num++;
        dataPoints->valuealarm_mode = gizStandardDecompressionValue(alarm_mode_BYTEOFFSET,alarm_mode_BITOFFSET,alarm_mode_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagtemperature_up)
    {
        info->event[info->num] = EVENT_temperature_up;
        info->num++;
        dataPoints->valuetemperature_up = gizX2YFloat(temperature_up_RATIO,  temperature_up_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valuetemperature_up)));
    }
        
    if(0x01 == issuedData->attrFlags.flagtemperature_down)
    {
        info->event[info->num] = EVENT_temperature_down;
        info->num++;
        dataPoints->valuetemperature_down = gizX2YFloat(temperature_down_RATIO,  temperature_down_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valuetemperature_down)));
    }
        
    if(0x01 == issuedData->attrFlags.flaghumidity_up)
    {
        info->event[info->num] = EVENT_humidity_up;
        info->num++;
        dataPoints->valuehumidity_up = gizX2YFloat(humidity_up_RATIO,  humidity_up_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valuehumidity_up)));
    }
        
    if(0x01 == issuedData->attrFlags.flaghumidity_down)
    {
        info->event[info->num] = EVENT_humidity_down;
        info->num++;
        dataPoints->valuehumidity_down = gizX2YFloat(humidity_down_RATIO,  humidity_down_ADDITION, (float)(exchangeBytes(issuedData->attrVals.valuehumidity_down)));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuealarm_mode != cur->valuealarm_mode)
    {
        GIZWITS_LOG("valuealarm_mode Changed\n");
        ret = 1;
    }
    if(last->valuetemperature_up != cur->valuetemperature_up)
    {
        GIZWITS_LOG("valuetemperature_up Changed\n");
        ret = 1;
    }
    if(last->valuetemperature_down != cur->valuetemperature_down)
    {
        GIZWITS_LOG("valuetemperature_down Changed\n");
        ret = 1;
    }
    if(last->valuehumidity_up != cur->valuehumidity_up)
    {
        GIZWITS_LOG("valuehumidity_up Changed\n");
        ret = 1;
    }
    if(last->valuehumidity_down != cur->valuehumidity_down)
    {
        GIZWITS_LOG("valuehumidity_down Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(buzzer_BYTEOFFSET,buzzer_BITOFFSET,buzzer_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebuzzer);
    gizStandardCompressValue(relay_BYTEOFFSET,relay_BITOFFSET,relay_LEN,(uint8_t *)devStatusPtr,dataPoints->valuerelay);
    gizStandardCompressValue(alarm_mode_BYTEOFFSET,alarm_mode_BITOFFSET,alarm_mode_LEN,(uint8_t *)devStatusPtr,dataPoints->valuealarm_mode);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));


    devStatusPtr->valuetemperature_up = exchangeBytes(gizY2XFloat(temperature_up_RATIO,  temperature_up_ADDITION, dataPoints->valuetemperature_up));     
    devStatusPtr->valuetemperature_down = exchangeBytes(gizY2XFloat(temperature_down_RATIO,  temperature_down_ADDITION, dataPoints->valuetemperature_down));     
    devStatusPtr->valuehumidity_up = exchangeBytes(gizY2XFloat(humidity_up_RATIO,  humidity_up_ADDITION, dataPoints->valuehumidity_up));     
    devStatusPtr->valuehumidity_down = exchangeBytes(gizY2XFloat(humidity_down_RATIO,  humidity_down_ADDITION, dataPoints->valuehumidity_down));     
    ...
    devStatusPtr->valuehumidity = exchangeBytes(gizY2XFloat(humidity_RATIO,  humidity_ADDITION, dataPoints->valuehumidity));     
