...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagfortify)
    {
        info->event[info->num] = EVENT_fortify;
        info->num++;
        dataPoints->valuefortify = gizStandardDecompressionValue(fortify_BYTEOFFSET,fortify_BITOFFSET,fortify_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuefortify != cur->valuefortify)
    {
        GIZWITS_LOG("valuefortify Changed\n");
        ret = 1;
    }
    if(last->valueinf_tube != cur->valueinf_tube)
    {
        GIZWITS_LOG("valueinf_tube Changed\n");
        ret = 1;
    }
    ...
    if(last->valuealaram_flag != cur->valuealaram_flag)
    {
        GIZWITS_LOG("valuealaram_flag Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(fortify_BYTEOFFSET,fortify_BITOFFSET,fortify_LEN,(uint8_t *)devStatusPtr,dataPoints->valuefortify);
    gizStandardCompressValue(inf_tube_BYTEOFFSET,inf_tube_BITOFFSET,inf_tube_LEN,(uint8_t *)devStatusPtr,dataPoints->valueinf_tube);
    gizStandardCompressValue(pyr_BYTEOFFSET,pyr_BITOFFSET,pyr_LEN,(uint8_t *)devStatusPtr,dataPoints->valuepyr);
    gizStandardCompressValue(alaram_flag_BYTEOFFSET,alaram_flag_BITOFFSET,alaram_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuealaram_flag);
