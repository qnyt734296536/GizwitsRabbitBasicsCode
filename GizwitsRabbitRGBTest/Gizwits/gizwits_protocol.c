...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagR)
    {
        ...
        info->num++;
        dataPoints->valueR = gizX2Y(R_RATIO,  R_ADDITION, exchangeBytes(issuedData->attrVals.valueR));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagG)
    {
        ...
        info->num++;
        dataPoints->valueG = gizX2Y(G_RATIO,  G_ADDITION, exchangeBytes(issuedData->attrVals.valueG));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagB)
    {
        ...
        info->num++;
        dataPoints->valueB = gizX2Y(B_RATIO,  B_ADDITION, exchangeBytes(issuedData->attrVals.valueB));
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueR = exchangeBytes(gizY2X(R_RATIO,  R_ADDITION, dataPoints->valueR)); 
    devStatusPtr->valueG = exchangeBytes(gizY2X(G_RATIO,  G_ADDITION, dataPoints->valueG)); 
    devStatusPtr->valueB = exchangeBytes(gizY2X(B_RATIO,  B_ADDITION, dataPoints->valueB)); 
