...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagcontrol_mode)
    {
        info->event[info->num] = EVENT_control_mode;
        info->num++;
        dataPoints->valuecontrol_mode = gizStandardDecompressionValue(control_mode_BYTEOFFSET,control_mode_BITOFFSET,control_mode_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagrgb_r)
    {
        ...
        info->num++;
        dataPoints->valuergb_r = gizX2Y(rgb_r_RATIO,  rgb_r_ADDITION, exchangeBytes(issuedData->attrVals.valuergb_r));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagrgb_g)
    {
        ...
        info->num++;
        dataPoints->valuergb_g = gizX2Y(rgb_g_RATIO,  rgb_g_ADDITION, exchangeBytes(issuedData->attrVals.valuergb_g));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagrgb_b)
    {
        ...
        info->num++;
        dataPoints->valuergb_b = gizX2Y(rgb_b_RATIO,  rgb_b_ADDITION, exchangeBytes(issuedData->attrVals.valuergb_b));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuepyr != cur->valuepyr)
    {
        GIZWITS_LOG("valuepyr Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(control_mode_BYTEOFFSET,control_mode_BITOFFSET,control_mode_LEN,(uint8_t *)devStatusPtr,dataPoints->valuecontrol_mode);
    gizStandardCompressValue(pyr_BYTEOFFSET,pyr_BITOFFSET,pyr_LEN,(uint8_t *)devStatusPtr,dataPoints->valuepyr);
    ...
    devStatusPtr->valuergb_r = exchangeBytes(gizY2X(rgb_r_RATIO,  rgb_r_ADDITION, dataPoints->valuergb_r)); 
    devStatusPtr->valuergb_g = exchangeBytes(gizY2X(rgb_g_RATIO,  rgb_g_ADDITION, dataPoints->valuergb_g)); 
    devStatusPtr->valuergb_b = exchangeBytes(gizY2X(rgb_b_RATIO,  rgb_b_ADDITION, dataPoints->valuergb_b)); 
